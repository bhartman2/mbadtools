...
# Packages {-}
The `devtools` package will be installed, if you have not already done so, and loaded. You only need to do this once in the term. 
```{r}
...
The `mbad_standard_packages` object contains a list of required packages for this course that you will need to install first. You only need to see this if you are unsure which packages have been installed. 
```{r}
data("mbad_standard_packages")
mbad_standard_packages
```

This is the key step: load the `mbadtools` package and then the standard packages using the `lib_packages()` function. You will use this in every new notebook!!
```{r}
