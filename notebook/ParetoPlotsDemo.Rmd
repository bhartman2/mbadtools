...
A2 %>% pareto_plot(cyl, c_count) +
  pareto_sec_axis(mc)
```
Add labels. You can control them with `nudge_y` and `size`.
```{r}
mc = max(A2$c_count)
A2 %>% pareto_plot(cyl, c_count) +
  pareto_sec_axis(mc) +
...
```{r warning=FALSE}
library(readr)
airline_2m <- read_csv("airline_2m.csv")
# airline_2m
```

```{r}
airline_delay = airline_2m %>% 
  select(FlightDate,
         Reporting_Airline,
         Origin,
         OriginState,
...
Pareto chart of departure delay DepDelay by Reporting_Airline. Notice our numeric variable isn't a count, but an average delay.

```{r}
airdel = airline_delay %>% 
  select(Reporting_Airline, DepDelay) %>% 
  group_by(Reporting_Airline) %>% 
  summarize(delays=mean(DepDelay, na.rm=T))
airdel %>% skim

AD = airdel %>%  pareto_arrange_data(delays)
AD
```

Set the number in `head()` to give just more than 80%.
```{r}
AD %>% head(23) %>% 
  pareto_plot(Reporting_Airline, delays) +
  pareto_pct_labels(max(AD$delays)) +
  pareto_sec_axis(max(AD$delays)) +
  pareto_cutoff_line(max(AD$delays)) +
  labs(title="Pareto Chart of average delays by Airline",
       x="Airline Code",
       y="Average of delays")
```

Not really any extremely bad performers.
...
ContainerFlows <- read_excel(
  "API_IS.SHP.GOOD.TU_DS2_en_excel_v2_1657.xls",
  skip = 3)
ContainerFlows %>% skim()
```
...
Pareto data frame: use mean of flow over the 10 years for each country. Change mean to millions of containers.
```{r}
...
df1 %>% gt() %>% fmt_number(decimals=3) %>% opt_interactive()
mc = max(df1$cflow)
```

Pick a number for about 80% and make Pareto Plot.
```{r}
q=25
C = df1 %>% head(q) %>% 
  ...
  pareto_cutoff_line(mc) +
  pareto_sec_axis(mc)
list(C) %>% wrap_plots(nrow=1, ncol=1) 
```

We don't know country codes, so extract the names and make a `gt` table.
```{r}
...
w = c %>% select(-name)
ww = w %>% filter(Country %in% (df1 %>% head(q) %>% pull(Country)) )
ww %>% gt() %>% opt_interactive()
```
