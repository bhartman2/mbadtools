...
```{r}
ggplot(moody) + 
  geom_pointline(aes(Date,ICS, color="Consumer Confidence")) +
  geom_pointline(aes(Date,Wgrowth*25, color="Wage Growth")) +
  # labs(y="ICS and Wage Growth*25") +
  scale_color_manual(values=c("red","black"), name="")+
  theme(legend.position = "bottom") +
  scale_y_continuous(name = "Consumer Confidence Index", 
...
## `Wgrowth` =G=> `ICS`
```{r}
grangertest(ICS~Wgrowth, data=moody, order=4)
```

`Wgrowth` does not Granger-cause `ICS.`

# `mbadtools` function
We will tune the order parameter by choosing different lags of 1 through 10 for the Granger-causality test. We'll run both directions of causality, and plot the results. Package `mbadtools` contains a function to test for all lags up to a number given by `max.lags`, which defaults to 10.
...
# EU Stock Markets
This dataset contains time series of four indices from the EU stockmarkets. Does any of them Granger-cause another? We will test `FTSE` and `DAX`.
```{r}
data(EuStockMarkets)
