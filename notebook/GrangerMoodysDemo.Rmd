...
# Data
The next sections show how to get the data for the Moody report. But we have already installed the data required in the `mbadtools` package as dataset `moody`.
...
## AFWT
```{r warning=FALSE}
waget = wage_growth_data %>% dplyr::select(1,2) %>% rename(Date=1, Wgrowth=2) %>%
...
umics1 = umics %>% 
  ...
  dplyr::select(Datep, ICS) #%>% 
umics2 = umics1 %>% mutate(Datep=Datep + days(1) + hours(6) + minutes(59))
waget1 = waget %>% dplyr::select(Date, Wgrowth)
...
# Granger Causality
We will test Granger causality between these two series, using 4 lags, with the `grangertest()` function. Its limitation is that you have to do one lag at a time, and one direction at a time.

## `ICS` =G=> `Wgrowth`
```{r}
grangertest(Wgrowth~ICS, data=moody, order=4)
```

Yes, it appears `ICS` does Granger-cause `Wgrowth` when 4 lags are used.
...
`Wgrowth` does not Granger-cause `ICS` with 4 lags.
...
```{r fig.height=7}
ICSt = moody  |>   dplyr::select(ICS)  |>  ts()
Wgrt = moody  |>  dplyr::select(Wgrowth) |>  ts()
GrangerTestPvals(ICSt, Wgrt)
GrangerTestTune(ICSt, Wgrt)  |>  wrap_plots(nrow=2)
```
