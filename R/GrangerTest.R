...
GrangerPlot = function (.data) {
  ...
  gpd1 = ggplot2::ggplot(d) +
    ...
    ggplot2::geom_smooth(ggplot2::aes(lag, .data[[nm[2]]], color="loess fit"),
  ...
  gpd2 = ggplot2::ggplot(.data) +
    ggh4x::geom_pointpath(ggplot2::aes(lag, .data[[nm[3]]], color="P-values"),
                          linewidth=1.2) +
    ggplot2::geom_hline(
      ggplot2::aes(yintercept=.05, 
                   color="Significance level 95%"),
    ...
    ggplot2::labs(title=nm[3], 
                  x="Lag Order of Granger test", 
                  y="Significance (p-value)") +
    ggplot2::geom_smooth(ggplot2::aes(lag, .data[[nm[3]]], color="loess fit"),
                         method="loess", formula='y~x', linewidth=.7) +
    ggplot2::scale_color_manual(values=c("blue","black", "red")) +
...
GrangerTable = function(.data, s1, s2, max.lags=10) {
  ...
  n1 = paste0(rlang::as_label(rlang::enquo(s1)), " =G=> ", 
              rlang::as_label(rlang::enquo(s2)))
  n2 = paste0(rlang::as_label(rlang::enquo(s2)), " =G=> ", 
              rlang::as_label(rlang::enquo(s1)))
