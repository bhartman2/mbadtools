...
library(lmtest)
library(qcc) # for some pareto plot tools
```

# Data 
Package `qcc` has several datasets with examples of Pareto Plots using their functions.

```{r}
data(mtcars, package="qcc") # from qcc package
mtcars
...
A1
```
Now apply `pareto_arrange_data()` function to A1.
...
Now the basic plot.
```{r}
PPlot = A2 %>% pareto_plot(cyl, c_count)
PPlot
```
...
chickwts %>% group_by(feed) %>% 
  summarize(avwt = mean(weight)) %>%
      pareto_arrange_data(avwt) %>%
        pareto_plot(feed, avwt) +
        labs(x="feed",
             y="average weight")
```
